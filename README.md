...
- **Subject-addressed review tasks.** A review task can be raised against a dossier, tracked through open → in progress → resolved, and worked by anyone who can open the filing (`POST`/`GET`/`PATCH /review-tasks`). Spectroscopy sessions keep their own richer session-scoped review surface with per-session reviewer roles.
...
- **Team-owned campaigns with review tasks.** A campaign belongs to its creator's team rather than to whoever created it (one created without a team stays creator-only), and a review task can be raised against it and worked by anyone who can open it — the same `/review-tasks` surface Regentry uses, addressed by subject rather than by spectroscopy session.
