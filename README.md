...
> **License.** MolTrace is **source-available, not open source.** The code is published under the [Business Source License 1.1](LICENSE) so you can read, audit, evaluate, and test it — but **production use requires a commercial license** (contact [moltrace.co@gmail.com](mailto:moltrace.co@gmail.com)). Each version converts to Apache 2.0 four years after release. See [`LICENSE`](LICENSE), [`NOTICE`](NOTICE), [`SECURITY.md`](SECURITY.md), and [`CONTRIBUTING.md`](CONTRIBUTING.md).
...
The compliance floor ships with **every** configuration, which is the point of it: the audit chain, e-signatures, controlled records, validation and the compound registry are platform, not per-product. The audit trail is verifiable from outside the system, not only from inside it — a scientist can check the chain behind one filing under that filing's own access rule, export those entries in the exact shape their hashes cover, and re-derive the whole chain with a standalone script that imports nothing from MolTrace. Anchors are Ed25519-signed where configured, so an auditor confirms a checkpoint is ours using a **public** key; the older symmetric scheme could only ever be checked with a key that also forges it, and the tool says so rather than implying otherwise. Collaboration is subject-addressed rather than spectroscopy-only — review tasks, comments, sign-off decisions and reviewer nominations can be raised against a regulatory filing or a reaction campaign, authorized by that subject's own access rule. Filings and campaigns are owned by a team rather than solely their creator, so regulatory affairs and process chemistry work as the group activities they are.
...
- **Chemical-shift prediction** from the NMRNet SE(3)-equivariant model — either a lazily-loaded local torch backend or a GPU inference sidecar, so the torch-free API host can still reach it — behind an always-available HOSE-code topological fallback, via `POST /spectrum/predict/shifts`. Which backend answered is reported on every result, and so is **how far the fallback had to reach**: each shift carries its source (model, matched environment, or bare element prior) and each prediction carries the share of atoms that matched nothing plus the median uncertainty per nucleus. That last number is the honest one — with only the bundled curated seed table a drug-like molecule resolves poorly and the median ¹³C uncertainty is far too wide to discriminate candidates, so the fallback's knowledge base is built from an assigned NMReDATA/NMRShiftDB2 export (`scripts/build_hose_kb.py`) rather than assumed. Published model accuracy figures describe the NMRNet path only and are never applied to a fallback result.
- **LC-MS / MS/MS evidence studios** with CSI:FingerID (MS/MS → structure via SIRIUS, optional), retention-time corroboration, and DP4 posterior scoring (Smith & Goodman 2010 σ/ν); candidates fuse into one ranking that orders the candidates supplied — decision support, not a calibrated probability that the leading candidate is correct.
...
- **Team-owned dossiers.** A dossier belongs to its creator's team, so a regulatory group shares one filing instead of one login (a system key or admin is unrestricted); a dossier created without a team stays creator-only. Anyone outside gets a non-leaking 404 — indistinguishable from a dossier that does not exist — so filing evidence stays isolated and cannot be probed for.
- **Subject-addressed collaboration.** A filing carries four collaboration surfaces addressed by subject rather than by spectroscopy session, gathered into one **Collaboration** section on the dossier workspace: **review tasks** tracked open → in progress → resolved (`POST`/`GET`/`PATCH /review-tasks`); **notes** that can be marked settled without being deleted (`POST`/`GET /comments`, `PATCH /comments/{id}`); **sign-off decisions**, which are never edited — changing position means recording another decision, and the newest one stands (`POST`/`GET /approvals`); and **reviewer nominations** recording who is expected to look (`POST`/`GET /reviewers`). Two boundaries are deliberate and stated in the interface: a nomination records an expectation and does **not** grant access — access comes from the owning team, so nominating someone outside it succeeds and lets them in nowhere — and a sign-off decision is **not** an electronic signature, which is bound to a point-in-time report through the e-signature surface (§11.70). A subject the caller cannot reach answers 404 exactly as one that does not exist does, so none of the four can be used to probe for another organization's filing. Spectroscopy sessions keep their own richer session-scoped surfaces, where a note can anchor to a specific piece of evidence and a reviewer row confers a session role.
...
Turns regulatory action items into reaction-optimization constraints: Bayesian, ML-guided next-experiment recommendations that carry the impurity limits applying to them, checked against measured results once an experiment is recorded.
...
- **Regulatory constraints from Regentry** — a dossier's ICH impurity action items are injected as reaction constraints, and a project's recorded experiment outcomes are evaluated against those limits (`…/regulatory-compliance`), flagging non-compliant experiments with provenance back to the source action item (`nmrcheck/reaction_regulatory_constraints.py`; a high/critical limit → non-compliant, lower tiers → an advisory penalty). Each ranked BO candidate also carries the verdict for the limits that apply to it, with its basis and a link back to the source action item, and the run reports how many candidates survived every filter. The scope is stated rather than implied: the surrogate scores a candidate on one scalarized objective, so it predicts none of the per-field values a limit is written against — those limits are reported as **not checked** on the proposal, never as passed, and are enforced against measured results. The closed loop: regulatory action item → reaction constraint → flagged result → next experiment.
...
- **Team-owned campaigns with the same collaboration layer.** A campaign belongs to its creator's team rather than to whoever created it (one created without a team stays creator-only), and it carries the same four subject-addressed surfaces Regentry uses — review tasks, notes, sign-off decisions and reviewer nominations — in one **Collaboration** section on the campaign workspace, worked by anyone who can open it.
...
- **Desktop** (`moltrace_desktop/`) — an Electron shell, in preview. It carries the platform's own navigation — the same six groups and three modules the web product defines — and shows every section whether or not this installation can run it, because a section that disappears reads as a feature the product does not have. Of the twenty, **one** has offline substance: SpectraCheck, because the local service *is* the science. The rest name what is missing in a sentence a chemist can act on ("the rule engine is not on this computer"), and Regentry and Repho are stated as **absent** rather than locked, which is the truth — their code is not in the build. Within SpectraCheck it reads a spectrum, measures it, checks a proposed structure against it, counts protons against that structure, takes a mass-spectrum peak table and a 2-D cross-peak table so **all four** of the verifier's tests run rather than two — the 2-D being the only one that can argue back, since its weight comes from how many correlations the structure predicts rather than how many the data matched, so a wrong structure is pushed down rather than merely left unsupported, ranks candidates, and looks the spectrum up against reference spectra shipped with the build — all with no network call. It reads a spectrum on the machine it runs on: the shell starts a packaged copy of the science service as a local child process and talks to it over a **Unix domain socket the shell creates itself**, owner-only on both the socket and its directory, with a fresh 256-bit credential per launch that is passed on an inherited descriptor rather than through argv or the environment, accepted in one named header, and refused in a query string, a path, a cookie or a body. The renderer never sees that credential and never names a file: it asks the main process to open a spectrum and receives a summary back. Analysis runs with **no network call at all** — the offline policy table classifies reading and processing an acquisition as local computation, and an operation absent from that table raises rather than defaulting. Results are reported as **multiplets rather than raw fitted lines**, because the peak detector over-picks and a line list misstates how many signals a spectrum contains; areas are the **integral of the trace** under each signal, over windows built from each line's own centre and width and bounded by the midpoints between neighbouring signal centres so they tile the spectrum rather than overlap — bounding them by their *edges* instead is what the first version did, and it inverts a window that sits entirely inside a broader neighbour's, silently reporting that signal as 0.0% of the spectrum — never a sum of fitted peak areas, which do not partition anything: the desktop fits each apex independently, a line may be fitted twice as wide as the data constraining it, and summing such fits inflated one acquisition's denominator 2.15× and reported a three-proton methyl as 1.4. They are shown as a share of the signals listed rather than as proton counts, since a proton count needs a denominator only a structure supplies — and once one is supplied that share becomes a count, scaled to the structure's **non-exchanging** hydrogens because OH, NH and SH swap with the solvent and often are not seen at all. That readout states what it cannot show: the counts add up to the structure's own hydrogen total *because the scale was chosen to make them*, so the bottom line agrees for any structure of the same hydrogen count and is not evidence for this one. The information is in the per-signal residual — a signal at 1.4 H is telling you two environments merged, one was split in two, or the structure is wrong — and in an expected-against-observed table whose rows are regions of the shift axis rather than assignments. It also states what it excluded from the denominator, because that is what most often makes these numbers wrong: on one reference acquisition **48.7%** of the listed area is classified as solvent or impurity — including a nine-line coupled multiplet sitting where water appears, water being a single line — and the panel names each such signal rather than reclassifying it, since a residual solvent can be coupled too — and the caveat names that denominator rather than the spectrum's own integral, which it is measurably not (0.042× to 2.150× of it across the reference acquisitions). Each result carries its own limits, emitted by the engine so an interface cannot render the numbers bare. The window opens and reports honestly whatever the service does, including when it fails to start. The structure check runs `verify_structure` itself — the platform's deterministic arbiter, four tests combined through a stated Bayesian update — and the candidate ranking is Smith & Goodman DP4 over the candidates supplied. Both are fed by a **495,215-atom NMRShiftDB2 reference table shipped inside the build**, and the difference that makes is not a nuance: without it the predictor answers from a 16-molecule seed at ~35 ppm median ¹³C uncertainty against ~1.9 ppm, and on a known spectrum it ranked the wrong molecule first. Every figure states what it is not — the DP4 share is labelled a relative ranking and never a calibrated probability, the library lookup prints its own measured hit rate, and prints the one that describes what the reader just did rather than the flattering one — querying with a real acquisition's measured signals, the true compound came back first in 3 of the 15 cases where it is in the library at all and inside the top five in 4, against the 48%/63% a record-against-record leave-one-out had reported for a different task, a ranking whose top two do not survive resampling within the acquisition's own resolution says so — including when the leader *changes* rather than merely narrows, which reading the sorted margin alone cannot see, and including the case where only one candidate matched any measured signal at all, which scores a perfect 100% margin by having no opponent rather than by winning, and a signal below the limit of quantitation is denied a multiplicity and a coupling rather than being shown one. That table and the reference spectra are NMRShiftDB2 derivatives under **CC BY-SA**: the licence ships inside the artifact beside the data, and packaging refuses a build that carries either without it. Its source is here for the same reason the rest is: shipping an Electron application publishes its client regardless, since ASAR is an archive rather than encryption, so what protects the product is the licence, the backend and the signature — not secrecy. `src/product.json` is checked in and **inert** — every operational value is `null`, so a build from this repository reaches no workspace, no update feed and no licensing authority, and refuses to start rather than falling back to a default. Real configuration is a private overlay applied at package time. Signing identities, notarization credentials and every entitlement key are absent by design: the client verifies signed entitlement statements against a pinned public key and never mints one.
...
**The AI model lifecycle** (`moltrace.spectroscopy.ai/eval/data/feedback/ops`) adds a versioned append-only model registry, a provenance-emitting inference router (LoRA fine-tuned → NMRNet pretrained → deterministic HOSE fallback) that records per-prediction the layer used and why, an evaluation harness with a dominance gate over a checksum-locked gold set, LoRA fine-tuning, Bayesian HPO (Optuna), confidence calibration with ECE as a promotion gate, closed-loop RLHF/A-B testing (a Bradley–Terry reward model that only re-ranks *within* a verifier verdict class), active learning, and a fail-closed four-check deployment gate (dominance / audit-chain / tests-green / data-leakage) wired into CI. That lifecycle reaches the product through one import boundary (`nmrcheck.ai_engine_adapter`): the AI governance surface records what an engine computed rather than what a caller submitted, so a stored confidence, uncertainty and domain assessment always travel with the `model_versions` provenance (artifact id → SHA-256) of everything that produced them, and an engine that cannot run reports that no prediction was produced rather than substituting a number. Confidence is reported on the deterministic verifier's own uncertainty scale — a prediction at the reference σ scores 0.870, not 1.0 — and model promotion is gated by the harness's dominance rule, under which safety-critical metrics (calibration error, false-confirmation rate) carry zero regression tolerance, so an accuracy gain cannot buy a worse calibration. Shift prediction and DP4 candidate ranking are engine-backed today; the remaining registered services record submitted values and say so explicitly.
...
For local development, run the backend on `:8000` and the frontend on `:3000`. The desktop shell is driven from `moltrace_desktop/` with `npm test` and `npm start`; an unconfigured build refuses to start and names what it is missing. Its `npm test` covers the renderer-confinement invariants, the product-config and transport-credential rules, and a round-trip that drives a real click through to a rendered peak table against the local service — that last one needs a working `uv` environment and a reference acquisition, and says plainly that it asserted nothing when it cannot run.
...
| `ENTITLEMENT_ISSUING_PRIVATE_KEY` / `ENTITLEMENT_CERTIFICATE` / `ENTITLEMENT_CERTIFICATE_SIGNATURE` / `ENTITLEMENT_ROOT_PUBLIC_KEY` | backend | The deployment's own offline-licence signing sub-key and the MolTrace-signed authorisation binding it to this deployment. Set together or not at all — a half-provisioned issuer is a startup issue, not a refused licence discovered hours later. A deployment that licenses no offline installations leaves all four unset and starts normally. See [`moltrace_backend/docs/entitlement_provisioning.md`](moltrace_backend/docs/entitlement_provisioning.md). |
| `ENTITLEMENT_OFFLINE_PERIOD_DAYS` / `ENTITLEMENT_STATEMENT_VALIDITY_HOURS` / `ENTITLEMENT_LICENCE_CLASS` | backend | Commercial terms carried inside the signature. **No defaults, deliberately** — none has been measured, and a plausible round number nobody chose would be signed into every statement. Unset means the deployment declines to issue and says which decision is missing. |
...
- **Security.** Report suspected vulnerabilities **privately** — use GitHub's *Security → Report a vulnerability*, or email [moltrace.co@gmail.com](mailto:moltrace.co@gmail.com). Do **not** open public issues for security reports. See [`SECURITY.md`](SECURITY.md).
...
- **Production use requires a commercial license.** For commercial licensing, contact [moltrace.co@gmail.com](mailto:moltrace.co@gmail.com).
