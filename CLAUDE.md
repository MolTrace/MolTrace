...
- `app/api/backend/[...path]/route.ts` is a same-origin proxy to FastAPI. The browser **always** calls `/api/backend/*`; `lib/api/client.ts` deliberately ignores an absolute `NEXT_PUBLIC_API_BASE_URL`. The proxy also **sanitizes 401/403 response bodies**. It matches on the body's **`code`** field — never on `detail` — against `PUBLIC_ERROR_CODES` in `route.ts`, a nine-member allowlist mirroring `error_codes.PUBLIC_CODES`: `module_not_licensed`, `step_up_required`, `token_expired`, `token_invalid`, `token_reuse_detected`, `product_not_in_plan`, `product_not_enabled`, `product_not_provisioned`, `role_required`. A recognised code is forwarded; an unrecognised one is sanitized, so it fails safe. **`detail` is replaced on every 401/403 regardless** — including when a public code is forwarded — because the backend's prose is human copy and matching it would make a copy edit a breaking API change. So **branch on `code` (or on status), never on `detail`**, and note the browser never sees the backend's `detail` for these statuses.
