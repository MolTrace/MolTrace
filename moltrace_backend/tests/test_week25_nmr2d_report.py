...
def test_2d_evidence_section_appears_in_json_and_html_report(client, api_headers) -> None:
    with client:
        analysis_id = _ethanol_analysis_id(client, api_headers)
        run_id = _save_linked_2d_run(client, api_headers, analysis_id)
...
def test_2d_report_includes_score_components_and_warnings(client, api_headers) -> None:
    with client:
        analysis_id = _ethanol_analysis_id(client, api_headers)
        _save_linked_2d_run(client, api_headers, analysis_id)
        report = client.get(f"/reports/{analysis_id}.json", headers=api_headers)
