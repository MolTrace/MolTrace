...
def test_retrieve_not_configured_is_graceful(client, monkeypatch):
...
def test_retrieve_configured_index_finds_match(client, tmp_path, monkeypatch):
...
def test_retrieve_smiles_mode(client, monkeypatch):
    monkeypatch.delenv("MOLTRACE_SIMILARITY_INDEX", raising=False)
    with client:
...
def test_retrieve_empty_query_is_400(client):
    with client:
        res = _post(client, {})
    assert res.status_code == 400


def test_retrieve_invalid_smiles_is_400(client):
    with client:
        res = _post(client, {"smiles": "not_a_smiles)("})
    assert res.status_code == 400


def test_retrieve_top_k_bounds_are_validated(client):
    with client:
...
def test_retrieve_registered_in_openapi(openapi_schema):
