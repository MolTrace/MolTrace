...
|---|---|---|---|---|---|
| `sample-a/10` | CDCl₃ | 30.0 s | 16 | yes | yes |
| `sample-a/11` | CDCl₃ | 5.0 s | 16 | yes | yes |
| `sample-b-…-5/6/7/8` | D₂O | 5.0 s | 128 | yes | no |
...
Verified on `sample-a/10`: Bruker zip detected, 131072 points, group-delay
...
Measured effect on `sample-a/10`: axis error corrected from **+0.024 ppm
...
> Run both arms over `sample-a/10` and `sample-b-…-5`, toggling
...
> this phase.
>
> Validate against `sample-a/10` (30 s recycle — the cleanest
...
> Drive the full loop on `sample-a/10`: upload → analyze → review →
...
`sample-a/10`, comparing raw trapezoid integration against the sum of
fitted line areas:
...
Driven on `sample-a/10` (the fully relaxed half of the matched pair),
