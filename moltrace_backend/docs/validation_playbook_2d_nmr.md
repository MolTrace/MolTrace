...
sulfamates, sample-c, tobramycin — structures known with certainty), but no 2D
was acquired.
...
sulfamate or sample-c would give ground truth no public corpus can match, because
...
2026-06-05 to 2026-08-06, on the `sample-d` / `sample-e` series — the same
pyrrolidine/sulfamate chemistry as the existing 1D fixtures.

**Confirmed 2D**, at `B500 / 06-05-2026 / sample-d-2 / 12`:
...
* **Experiment numbers do not indicate dimensionality.** `sample-d-2/12` is a
  COSY, but `sample-d-1/20` — a higher number in a later session — is
