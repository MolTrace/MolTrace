...
Regulatory Hub must NOT launch until every validation check is green: worked-example zero-error +
...
def enforce_launch_gate() -> None:
    ...
    if not result.passed:
        raise LaunchGateError(
            f"Regulatory Hub launch gate is RED — do not go live. Failed: {result.failed_checks()}"
        )
