...
Document version: 2026-Q2 · rev 2026-08-02 · Hybrid (business + technical) · Approx. 5,400 words
...
- **SpectraCheck** — multi-layer NMR + MS evidence engine that ingests 1H, 13C, 2D NMR, raw FID (Bruker / Agilent-Varian), HRMS, MS/MS, and LC-MS feature data; categorises peaks against literature-backed chemical-shift windows; ranks candidate structures with the Smith & Goodman DP4 likelihood — a relative ordering of the candidates supplied, not a calibrated probability of correctness; and surfaces every numerical claim with its provenance hash.
...
Crucially, the engine never asserts identity. The wording in the response is *"decision support, not proof of identity and not a calibrated DP4/DP5 probability."* The DP4 mathematics from Smith & Goodman (2010)[^smith_goodman_2010] — with the DP4-AI[^howarth_2020_dp4ai] and DP5[^howarth_2022_dp5] literature cited alongside it — is implemented and exposed as its own candidate-ranking panel, and that panel labels its own number for what it is: because DP4's error model describes DFT-computed shifts while MolTrace predicts shifts empirically, the value is a defensible relative ordering of the candidates supplied, not a calibrated probability that the leading one is correct. DP5 itself is cited background and near-term roadmap (§10.1), not a shipped scorer.
...
**Spectrum retrieval.** Complementing scoring and verification, a similarity-retrieval layer answers "which known molecules look like this spectrum?" — a Gaussian-smoothed 256-D encoding of the ¹H / ¹³C shifts with FAISS HNSW nearest-neighbour search (top-100 from a ~45k reference set in milliseconds), plus a Kuhn-Munkres set-similarity score robust to peak insertion/deletion. It follows the published NMR-Solver retrieval method,[^nmr_solver] implemented from the equations rather than any copyrighted text; an index derived from NMRShiftDB2 carries the CC-BY-SA ShareAlike obligation and is never committed. One design point is worth naming because it is where naive implementations quietly fail: **most public reference spectra record only one nucleus** — 82 % of the 42 449-molecule NMRShiftDB2 export — and a single combined index ranks those entries by which nucleus is *missing* rather than by chemistry, to the point where a carbon-only query cannot reach a molecule that also has proton data *at all*. MolTrace indexes each nucleus separately and compares a query to a reference only on the nuclei the two actually share, with a measured penalty for incomplete coverage so that partial records neither dominate nor disappear. A chemist who ran a single experiment therefore searches the whole reference set, not the fraction of it that happens to match their acquisition.

**An audit trail you can check without trusting us.** Most audit trails are verifiable only from inside the system that produced them, which is a weak form of the claim: the party asserting the record is intact is the same party that holds it. MolTrace's hash chain is built so an outsider can re-derive it. Every entry is linked by an **unkeyed** SHA-256 digest, so recomputing the whole chain needs no secret at all — a scientist can pull the entries behind one filing under that filing's own access rule, and a standalone script that imports nothing from MolTrace re-checks them and reports exactly which entry was altered, removed or reordered. Periodic checkpoints are signed with a private key whose **public** half is published, so an auditor can confirm a checkpoint is genuinely ours using material that cannot forge one. The tool is equally explicit about its limit: for checkpoints sealed under the older symmetric scheme it reports that authenticity *cannot* be established rather than implying it has been, because "could not verify" and "valid" must never read the same.
...
A campaign's chemistry enters the system as a **drawing**. Reaction Studio's canvas captures what a chemist draws as an MDL molfile or reaction block, and that capture is read by RDKit — the engine every other chemistry decision on the platform already runs on — rather than trusted from the browser's drawing engine, because two chemistry engines disagreeing *silently* is precisely the failure a regulated workspace cannot absorb. The check reports in plain language whatever reading the structure changed — hydrogen counts, formal charges, unpaired electrons — instead of sanitising quietly, and a drawing that fails its checks is still a successful question with the verdict in the answer rather than in an error: asking whether a structure is sound and hearing *no* is a result. A scheme attached to a campaign retains **both** forms — the drawing exactly as made, which is the audit record, and the normalised form downstream chemistry computes on — is owner-scoped, and is archived against a required reason rather than deleted. A drawn motif can also be matched against structures the chemist supplies; that match is bounded to exactly those structures and says so, and it reports anything the engine could not read as *unread* rather than as a miss, since "nothing matched" across a set that was never searched is the false negative a chemist would act on. Substructure search across the whole registry is a larger piece and is deliberately not what this answers.

Before a flagged chemistry reaches the bench, the program runs a **structural process-safety screen**: a deterministic RDKit substructure search for well-known energetic and reactive functional groups (organic azides, peroxides and peroxy-acids, diazo and diazonium species, poly-nitro motifs, perchlorates, tetrazoles, and related explosophores), each carrying a conservative severity and a mitigation note. The screen is decision-support only — never a safety determination — and is deliberately fail-safe: an unparseable or flagged structure is never silently cleared, and any screen above the lowest tier holds the project's safety gate at *review-pending* until a qualified process-safety professional records a verdict (a rejection hard-blocks). It encodes public GHS / structural-chemistry knowledge, not a licensed hazard dataset, and quantitative thermochemistry (exothermicity, gas evolution, DSC onset) is explicitly out of scope.

Two disciplines govern how the program is allowed to change. First, **no change to the recommender ships without measurement**: every candidate is scored against an immutable, content-checksummed benchmark, and the promotion gate is fail-closed — a result that cannot be tied to that exact benchmark is refused rather than compared, a missing metric blocks instead of defaulting to a favourable value, and a candidate that misses hazards the incumbent caught is unpromotable no matter how much its accuracy improved. Second, the heavy machine-learning techniques the field expects — graph neural networks for yield prediction, Monte-Carlo-tree-search retrosynthesis, transformer forward prediction, and self-driving-lab execution — ship as **optional, default-off capabilities behind a single governed seam**, not as a rewrite of the deterministic core. Each is switched on deliberately, one flag at a time; its dependencies are probed rather than imported, so a customer who does not want them carries none of their weight; a model that replaces frozen arithmetic must first present a benchmark pass **bound to** that benchmark and that model version; every enablement decision is recorded with the provenance to audit it later; and when a capability is off, the lightweight path it falls back to is named rather than silently substituted. The predictions themselves are never trusted on their own authority: a proposed retrosynthetic route and a predicted product are both re-screened by the same frozen safety and green engines that govern the manual path, and robotic execution sits behind arm/heartbeat/disarm interlocks, a bounds-checked execution envelope, and a tamper-evident hash-chained journal — with manual mode the default everywhere. Reaction training data is governed on the same terms: an unregistered dataset cannot be ingested, benchmark corpora are held out of training by construction, and licence-restricted commercial sources are never ingested or bundled.
...
The 2023 final ICH Q2(R2) guideline expanded the acceptance criteria for analytical method validation to explicitly address data integrity through the analytical lifecycle.[^ich_q2r2] MolTrace's audit-event ledger (`audit_events` table), immutable raw vault, recipe-hash-linked processing runs, and human-review release gate map directly onto the ALCOA+ principles (Attributable, Legible, Contemporaneous, Original, Accurate, plus Complete, Consistent, Enduring, Available). Reinforcing these: a regulated change to a controlled record captures its **reason-for-change in a queryable field** and is attributed to the authenticated principal (Attributable/Legible); a controlled record is never physically deleted — archiving is a **reversible-by-record soft delete** that retains the row with the actor, timestamp, and reason (Enduring); every regulated timestamp is **server-sourced** (Contemporaneous); and the raw vault's write-once guarantee can be configured to **fail closed** rather than degrade silently if a stored archive cannot be made read-only (Original). As the analytical-evidence handoff to the Regentry, the pipeline emits a deterministic ICH Q2(R2) report stub — sample / spectrum summary, result counts, and validation-characteristic slots (specificity, accuracy, precision, range) — that embeds the analysis content hash (§4.8) so the evidence is traceable to the exact run that produced it; the stub scaffolds, and never substitutes for, full method validation.
...
| Assess AI model risk | Transparent multiplier tables + DP4 ranking panel (labelled relative, not calibrated) as fallback |
...
**Guidance grounded in the source corpus.** Alongside the calculators, Regentry can search the regulatory-guidance corpus itself. A versioned, licence-aware ingestion pipeline indexes ICH, FDA, EMA, and WHO guidance — FDA guidance is US-government public domain and redistributable; ICH, EMA, and WHO texts are copyrighted and held for internal retrieval only, kept to minimal excerpts and always carrying a citation plus a link to the official source. A retrieval layer then answers a regulatory question grounded *only* in the retrieved guidance,[^rag_lewis] returning each passage with its source document, section, effective date, and the MolTrace engine it maps to. The grounding is deterministic-first by construction: the default path is purely extractive and uses no language model at all; the optional Claude synthesis is constrained to the retrieved text and forbidden from stating any specific regulated number — a PDE, TTC, or threshold defers to the deterministic engines above, never to the model — with a post-hoc check that flags any fabricated number or invalid citation. As with every Regentry output, the result is cited decision-support gated behind qualified review.
...
MolTrace provides software controls that **support** 21 CFR Part 11 workflows[^cfr_part11] — a cryptographic audit trail, electronic signatures, and access control (including per-organization OpenID Connect single sign-on with PKCE, just-in-time provisioning, an optional enforce-SSO mode, and SCIM 2.0 auto-provisioning/**deprovisioning** so an offboarded user's access is revoked from the identity provider — soft-disable with immediate session revocation, never deleting an audit-linked user — multi-factor authentication (RFC 6238 TOTP plus phishing-resistant WebAuthn/passkeys) with per-tenant enforcement and **step-up re-authentication required at the moment of signing and before admin actions**, short-lived access tokens with rotating single-use refresh tokens (reuse detection revokes the whole session family) and **immediate server-side revocation**, plus server-side per-user/per-tenant scoping decided by a centralized, **deny-by-default policy engine** (policy-as-code authorization — every access decision is made server-side from the authenticated principal, never from a client-supplied owner/tenant id, and a new endpoint inherits the gate by default); the unique user identification, contemporaneous re-authentication, and lifecycle-managed access controls Part 11 §11.10(d)/§11.200 call for, with user passwords hashed under **Argon2id** (a memory-hard KDF — 64 MiB, t=3 — with a unique salt and optional KMS-held pepper; legacy PBKDF2 hashes verify and upgrade transparently on next login) and classified secrets (IdP client secrets, MFA seeds) protected by **field-level envelope encryption** — a per-record AES-256-GCM data key wrapped by a KMS key-encryption key, with key rotation and a customer-managed-key (BYOK) seam — backed by **secure-SDLC CI gates** (secret-scanning with gitleaks in CI and at pre-commit, plus SAST with Semgrep and dependency-, license-, and infrastructure-as-code scanning with Trivy — critical findings block merge and deploy, while lower-severity findings are reported to a code-scanning dashboard and tracked to closure under triage SLAs), a **signed software supply chain** (a CycloneDX SBOM emitted per build for both the backend and the frontend, SLSA build provenance signed keylessly via Sigstore, and a verify-at-deploy gate that refuses to fire the production deploy hooks unless the provenance verifies — with provenance queryable per release; the attestation covers the source and dependency closure at the gated commit, the honest boundary for a platform-rebuild deploy model), a zero-trust CI/CD pipeline (every GitHub Action pinned to a commit SHA, least-privilege workflow tokens) with continuous IaC posture scoring and drift detection (a CSPM-style Trivy-config baseline that fails the build on any new misconfiguration), **per-tenant + per-route API rate limiting** (an in-app token-bucket limiter keyed by the authenticated principal or client IP per route — returning HTTP 429 with `Retry-After` and recording each throttle as a security event — with a complementary network-edge WAF documented as a deployment runbook) and a request-body-size guard, a published coordinated **vulnerability-disclosure policy** (an RFC 9116 `/.well-known/security.txt` file, safe harbor, and CVSS-rated severity/remediation SLAs shared with the scanning gates) backed by a STRIDE threat model and an annual / pre-release penetration-testing program whose findings are tracked to remediation evidence, and an API that enforces **HSTS and security response headers over TLS 1.3**) — layered over the §6.5 data-integrity primitives. A decorator can wrap any analysis layer so each result is written as an immutable, signed audit record capturing the operator, the UTC timestamp, the SHA-256 of the input and output, every method parameter, the software version, and — for AI-assisted layers — the **exact model-weight checksum** that produced it, so the result is reproducible and traceable. The audit-event ledger itself is a tamper-evident SHA-256 hash chain — each row carries the hash of the one before it over a canonical serialization, so any insert, edit, deletion, or reordering breaks recomputation — periodically sealed with organisation-keyed HMAC-signed anchors plus a signed high-water mark (so even truncation of the most-recent records is detectable), with an admin verification endpoint and a reconciliation job that detects and alerts on any break. A detection layer scans the security-event stream and the chain — impossible travel, privilege escalation, cross-tenant access, and audit-chain breaks — and ships high-severity alerts to a pluggable SIEM sink (structured stdout for the platform log drain, plus an optional webhook); shipping to a hosted SIEM and 24/7 on-call routing are the operational complement. An incident-response program — severity tiers, runbooks keyed to those detections with real in-product containment levers, and a GDPR Art. 33/34 breach-notification workflow whose notification-deadline engine reflects MolTrace's processor role (it notifies the customer-controller, who owns the 72-hour supervisory-authority clock) — ties detection to response; sending notices and the on-call rotation are operational, and the breach-notification controls are designed to support the customer's obligations, not discharge them. A backup / disaster-recovery program documents RTO/RPO targets and a scheduled restore drill, and a restore-integrity verifier re-runs the audit-chain verification against a restored database to prove it is intact and un-tampered — the audit ledger is the natural restore-integrity oracle (cross-region/immutable backup storage and running the restore are operational). A machine-checked control-evidence register maps each in-repo control to the SOC 2 Trust Services Criteria and ISO 27001 Annex A — validated so a deleted control breaks the build — and a customer Trust Center with a sub-processor register makes the posture auditable; SOC 2 and ISO 27001 are designed-to-support, not held (the continuous-compliance tooling, the audits, and the published Trust Center are operational). On privacy, a machine-readable personal-data map drives DSAR and right-to-erasure planning that assists the controller (Art. 28(3)(e)) rather than adjudicating for it, and that keeps the distinction the law turns on: clearing an identifier is **pseudonymisation**, so the record remains personal data and is reported as retained-and-restricted, never as erased. Where a legal obligation requires retention (Art. 17(3)) the record is restricted rather than destroyed, and identity **cannot** be erased from the append-only audit ledger at all — the chain hashes the identity fields, so a rewrite would be indistinguishable from tampering and would breach the Part 11 §11.10(e) retention duty; crypto-shredding (hash the ciphertext, destroy the per-subject key) is documented as the enabling seam rather than claimed as a capability. Data residency is stated with the same candour: MolTrace runs single-region with no tenant region pinning today. Electronic signatures are designed per 21 CFR Part 11.50 (the signature manifestation carries the signer's name, the date and time, and the meaning — authorship, review, approval, or responsibility) and 11.70 (each signature is bound to its specific record so it cannot be copied or transferred). The hosted platform's e-signature service applies the same controls server-side: the signer is taken from the **authenticated principal** (never a client-supplied name), the signature digest **binds a SHA-256 of the exact signed record** so it cannot be moved to a different record or version, a durable manifestation is rendered for the inspection copy, and a verification endpoint re-derives the digest (and re-snapshots the live record) to surface any tampering or post-signing change. Records carry a configurable retention floor (default seven years) and export to a deterministic, submission-ready report (PDF/A). **Critically, these controls *help customers meet* 21 CFR Part 11 — MolTrace does not claim the product is itself compliant. Full computerized-system validation, SOPs, and identity management remain the customer's responsibility**, and the data-integrity basis follows the FDA's ALCOA+ guidance.[^fda_data_integrity]
...
The Validation Center turns this into a **regenerable validation package** per release: the on-demand requirement→risk→test→execution traceability matrix (with coverage and gap analysis), **IQ/OQ/PQ evidence assembled from CI** (the operational-qualification slot populated from the release's automated-test summary; installation and performance qualification honestly marked customer-supplied rather than fabricated), the change-control state, and the release's content-bound approval signatures — bundled into one deterministic artifact that can be re-generated on every CI build or inspection. **Validated-state change control** (GAMP 5 §14 / Annex 11) is enforced in code: once a validation project is approved or attached to an approved/released system release, any change to it or its requirement/risk/test records requires a captured reason-for-change and is recorded on the §6.6 hash-chained audit trail, so the validated baseline cannot drift silently. As with the document skeleton, the package and the change-control gate **support** the customer's CSV — they accelerate evidence assembly and baseline control; they do not perform or replace the customer's qualification or sign-off.
