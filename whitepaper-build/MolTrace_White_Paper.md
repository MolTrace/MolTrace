...
Alongside the default 39-layer chain, an **opt-in Mestrenova-style Global Spectral Deconvolution (GSD) analysis backend** is available at the dedicated `POST /spectrum/analyze/gsd` endpoint. GSD performs single-pass peak detection with iterative pseudo-Voigt overlap resolution, classifies every peak into `compound | solvent | impurity | artifact | 13C_satellite` against the curated Fulmer / Gottlieb residual-solvent tables, and clusters multiplet lines into chemical-environment entries so the response can be consumed at either granularity. The GSD backend has cleared its strict production-promotion gate against the curated NMRShiftDB2 corpus (100 % solvent auto-detect, median compound-environment-count delta of 2 vs expert reference) and an HMDB-style multiplet-line-granularity corpus (95 % / 100 % within tolerance). Tenants opt in per request via a frontend toggle; the default `/spectrum/analyze` flow is unchanged so existing pipelines stay green. See the technical white paper §3.1 for the algorithm, validation framework, and per-fixture results.
