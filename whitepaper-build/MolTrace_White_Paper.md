...
- **Versioned output contract.** Every SpectraCheck analysis serialises to a schema-versioned contract whose canonical-JSON **content hash** (`sha256:…`) is the analysis's stable identity — the same fingerprint the ComplianceCore handoff and the ICH report stub (§6.1) embed for traceability.
