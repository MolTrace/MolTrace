...
| Authentication | Tenant-aware opaque bearer sessions · per-organization OpenID Connect SSO (PKCE · JIT provisioning · optional enforce-SSO) · SCIM 2.0 auto-provisioning/deprovisioning · MFA (TOTP + WebAuthn/passkeys) with per-tenant enforcement + step-up re-auth for signing/admin · role-based access |
