...
* **Interface — `predict_shifts(smiles, nuclei=('1H','13C')) -> ShiftPrediction`.** Pipeline: parse SMILES (RDKit) → add explicit hydrogens → 3D embed (`ETKDGv3` + `MMFFOptimizeMolecule`) → atom types + coordinates → backend inference. The result maps each atom index to a `predicted_ppm` + `uncertainty_ppm` with provenance, and names the `backend` actually used.
* **Primary backend — NMRNet (opt-in).** The SE(3)-equivariant Transformer of Xu et al. (*Nat. Comput. Sci.* 5, 292–300 (2025); doi:10.1038/s43588-025-00783-z), with a reported benchmark MAE of ≈ 0.181 ppm (¹H) / 1.098 ppm (¹³C). NMRNet ships as a research codebase (Uni-Mol-based) plus downloadable weights, not a pip-installable model, so MolTrace integrates it as an **optional, lazily-loaded** backend: in-process, or — recommended — as a **separate GPU microservice** that the torch-free main backend calls over HTTP (`nmrnet_client` + the `nmrnet_service/` scaffold). It activates only when weights + dependencies are configured (`MOLTRACE_NMRNET_MODULE` / `MOLTRACE_NMRNET_SERVICE_URL`) and **never fabricates a prediction**; a small conformance interface (atom symbols + 3D coordinates in, per-atom shift out) bridges the real checkpoint, and the featurisation handed to it is the same RDKit conformer the pipeline already builds.
* **Fallback — HOSE-code / NMRShiftDB2.** When NMRNet is unavailable, a topological nearest-environment predictor: each atom is encoded as a HOSE-style spherical environment code (spheres 1–6) and looked up in a knowledge base, **decreasing the sphere until a match is found** (most-specific → most-general); the prediction is the mean shift of the matching reference atoms and the uncertainty their spread, falling back to an element-level prior (flagged high-uncertainty) when nothing matches. RDKit has no built-in HOSE generator, so the code is implemented in-module. The bundled knowledge base is a curated literature seed (common solvents and functional groups); `load_knowledge_base()` ingests a full NMRShiftDB2 assignment export for production coverage.
* **Endpoint:** `POST /spectrum/predict/shifts` — request `{ smiles, nuclei }`, response `{ smiles, nuclei, backend, shifts: [{atom_index, element, nucleus, predicted_ppm, uncertainty_ppm, method, provenance}], shift_count, notes }`. The response always names the active backend and carries notes (e.g. why it fell back); each call emits a `spectrum.predict_shifts` audit event, matching the GSD / multiplet / integration telemetry.
* **Validation:** `tests/test_nmrnet_wrapper.py`, `test_nmrnet_client.py`, `test_qm9nmr.py`, `test_spectrum_predict_shifts_api.py`. The fallback's mechanism is validated directly (seed-KB recovery of literature shifts + the sphere-decreasing generalisation); the NMRNet path's paper-accuracy gate (QM9-NMR MAE within 30 % of the reported value, with the shipped shielding→shift converter) is implemented but **skipped until a real checkpoint and the QM9-NMR test set are present** — MolTrace asserts no measured model accuracy it has not actually run.
