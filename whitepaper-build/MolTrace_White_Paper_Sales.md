...
**What this is worth in an inspection.** An audit trail whose integrity can only be confirmed by the vendor is the vendor's assertion, not evidence. MolTrace's trail is re-derivable by a third party: the entries behind a filing export in the exact form their hashes cover, and a standalone checker — no MolTrace software, no secret — confirms nothing was altered, removed or reordered, naming the offending record if something was. Checkpoints are signed so their authenticity is confirmable from a **published public key**, which the older symmetric approach could never allow, because the key that verifies is the key that forges. The product says which of the two it has established rather than blurring them.
...
- **Candidate comparison** with Smith & Goodman DP4 Bayesian likelihood scoring — a relative ranking across the candidates you supply, not a calibrated probability that the leading one is correct — and a transparent per-class multiplier table for structural classes (carbohydrates, lipids, peptides, polymers, etc.).
...
| Assess AI model risk | Transparent multiplier tables + DP4 ranking panel (a relative ranking, not a calibrated probability) as cross-check |
