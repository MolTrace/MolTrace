...
**Tier 3 — Enterprise (annual).** Multi-site multi-tenant deployment, per-organization single sign-on (OpenID Connect federation with your identity provider — Okta, Entra ID, Ping, etc. — with just-in-time provisioning and optional enforce-SSO) and SCIM 2.0 directory sync that auto-provisions and **auto-deprovisions** users (offboarding in your IdP revokes MolTrace access and active sessions immediately), mandatory MFA with phishing-resistant passkeys (WebAuthn/FIDO2) or authenticator-app TOTP plus step-up re-authentication before signing and admin actions, federated tenant-private predicted-NMR models, dedicated regulatory-affairs onboarding for FDA AI Credibility Framework mapping, and an SLA-backed audit response.
