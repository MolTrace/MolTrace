...
export type SpectraNavSection = { value: string; label: string; desc: string }
export type SpectraNavGroup = { id: string; label: string; sections: SpectraNavSection[] }

type Props = {
  groups: SpectraNavGroup[]
  activeValue: string
...
/**
 * Two-tier section navigation for the SpectraCheck workspace.
 *
 * The workspace has thirteen sections. Presented as one flat strip they were
 * uniformly small and every one competed with the other twelve, so the strip
 * read as a single undifferentiated ribbon and the pipeline structure it
 * already encoded — orient, gather evidence, analyse, produce output — was
 * invisible. Splitting it in two lets the primary tier carry real visual weight
 * (that is the decision a reader makes first) while the secondary tier shows
 * only the siblings that belong to the stage they picked.
 *
 * The group a section belongs to is derived from `activeValue`, so deep links
 * and programmatic jumps land on the right primary tab without extra state.
 */
export function SpectraCheckSectionNav({ groups, activeValue, onSelect }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="min-w-0 overflow-x-auto [-webkit-overflow-scrolling:touch]">
        <div
          role="tablist"
          aria-label="SpectraCheck stages"
          ...
          {groups.map((group) => {
            ...
            return (
              <button
                ...
                aria-selected={on}
                tabIndex={on ? 0 : -1}
                data-testid={`spectracheck-stage-${group.id}`}
              ...
              >
                {group.label}
                {/* Deliberately no section count here. A number on a tab reads as
                    a notification badge — items waiting on you — so "Evidence
            ...
            {activeGroup.sections.map((section) => {
              ...
              return (
                <button
                  ...
                  aria-selected={on}
                  tabIndex={on ? 0 : -1}
                  data-testid={`spectracheck-tab-${section.value}`}
                  onClick={() => onSelect(section.value)}
                  className={cn(
                    "inline-flex min-h-9 shrink-0 items-center whitespace-nowrap rounded-full border px-3.5 py-1.5 font-mono text-[13px] transition-colors",
                    "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1",
                    on
                  ...
                  style={on ? { backgroundColor: "var(--mt-teal)" } : undefined}
                >
                  {section.label}
                </button>
              )
