...
/**
 * Smoke test for the Regentry "Explore Module" overlay.
 *
...
async function switchToRegulatory(page: Page) {
  ...
  await page.locator("text=Regentry").first().waitFor({ timeout: 10_000 })
...
async function clickExploreOnActive(page: Page) {
  ...
  await page
    .locator('[role="region"][aria-label*="Regentry"]')
    .first()
...
async function main() {
  ...
  r = await safe(() =>
    page.locator("text=/Regentry · Live preview/i").first().waitFor({ timeout: 10_000 }),
  )
  record("Eyebrow 'Regentry · Live preview' rendered", r.ok ? "pass" : "fail", r.ok ? undefined : r.error)
