...
/**
 * Smoke test for the redesigned Regulatory Hub surfaces.
 *
...
const SPECS: Spec[] = [
  {
    ...
    eyebrow: /MolTrace · Regulatory Hub/i,
    heading: /^Regulatory Hub$/i,
  },
  {
...
async function main() {
  ...
  const md = [
    `# Regulatory Hub redesign — smoke ${new Date().toISOString()}`,
    "",
    `- Pass: ${passes}`,
    `- Fail: ${fails}`,
    "",
