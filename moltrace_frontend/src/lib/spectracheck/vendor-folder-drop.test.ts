...
/** Layout of a real Bruker acquisition folder (mirrors the sample-c test data). */
const BRUKER = [
  "sample-c/34/fid",
  "sample-c/34/acqus",
  "sample-c/34/acqu",
  "sample-c/34/pulseprogram.precomp",
  "sample-c/34/pdata/1/procs",
]
...
describe("vendor dataset detection", () => {
  it("recognises a Bruker folder by fid + acqus in the same directory", () => {
    ...
    expect(d.experiments[0].dir).toBe("sample-c/34")
  ...
  it("ignores OS junk so it neither counts nor reaches the archive", () => {
    const d = detectVendorDataset([
      entry("sample-c/34/fid"),
      entry("sample-c/34/acqus"),
      entry("sample-c/.DS_Store"),
      entry("sample-c/._fid"),
...
describe("archive naming", () => {
  it("names the archive after the dropped root folder", () => {
    expect(archiveNameForEntries(BRUKER.map((p) => entry(p)))).toBe("sample-c.zip")
  })
...
describe("client-side zipping", () => {
  it("produces a zip that PRESERVES relative paths (the backend groups members per directory)", async () => {
    ...
    expect(file.name).toBe("sample-c.zip")
    ...
    expect(members).toContain("sample-c/34/fid")
    expect(members).toContain("sample-c/34/acqus")
    expect(members).toContain("sample-c/34/pdata/1/procs")
  })
  ...
  it("omits OS junk from the archive", async () => {
    ...
    expect(members.sort()).toEqual(["sample-c/34/acqus", "sample-c/34/fid"])
  })
  ...
  it("reports progress and refuses a folder with nothing usable", async () => {
    ...
    await expect(zipVendorFolder([entry("sample-c/.DS_Store")])).rejects.toThrow(/no usable files/i)
  })
})
...
describe("splitting a dropped folder into one archive per experiment", () => {
  ...
  it("keeps a Bruker experiment's nested pdata files with their experiment", () => {
    ...
    expect(bundle.entries.map((e) => e.path)).toContain("sample-c/34/pdata/1/procs")
  ...
  it("keeps OS junk out of the bundles and out of their counts", () => {
    const [bundle] = splitVendorFolderByExperiment([
      entry("sample-c/34/fid"),
      entry("sample-c/34/acqus"),
      entry("sample-c/34/.DS_Store"),
      entry("__MACOSX/Nap/34/fid"),
    ])
    expect(bundle.entries.map((e) => e.path).sort()).toEqual(["sample-c/34/acqus", "sample-c/34/fid"])
  ...
  it("zips one bundle at a time into an archive the server can still read", async () => {
    ...
    expect(file.name).toBe("sample-c_34.zip")
    const members = Object.keys(unzipSync(new Uint8Array(await file.arrayBuffer()))).sort()
    expect(members).toEqual(["sample-c/34/acqu", "sample-c/34/acqus", "sample-c/34/fid", "sample-c/34/pdata/1/procs", "sample-c/34/pulseprogram.precomp"])
  })
})

describe("per-experiment archive naming", () => {
  it("flattens the directory path so the name says which experiment it is", () => {
    expect(experimentArchiveName("sample-c/Raw/34")).toBe("Nap_Raw_34.zip")
  })
...
describe("folder picker input", () => {
  it("uses webkitRelativePath so a picked folder keeps its structure", () => {
    ...
    Object.defineProperty(f, "webkitRelativePath", { value: "sample-c/34/fid" })
    expect(vendorFolderEntriesFromFileList([f])[0].path).toBe("sample-c/34/fid")
  })
